...
import { Check, X, Loader2, RefreshCw, ExternalLink, Eye, EyeOff, Cpu } from 'lucide-react';
import { useZAi } from '@/api/hooks';
import { cn } from '@/utils/cn';

interface ZAiSettingsPanelProps {
  readonly footer?: React.ReactNode;
  readonly onModelsSynced?: () => void;
}

export function ZAiSettingsPanel({
  footer,
  onModelsSynced,
}: ZAiSettingsPanelProps) {
  const {
    config,
    loading,
    testing,
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-medium text-gray-200">Z.ai</h3>
        </div>
        ...
        <form
          ...
          className="space-y-2"
        >
          <label htmlFor="zai-api-key" className="block text-sm font-medium text-gray-300">
            API Key
            <a
              href="https://platform.z.ai/"
              target="_blank"
          ...
          <div className="relative">
            <input
              ...
              placeholder={hasApiKey && !masked ? undefined : 'sk-...'}
              className="w-full px-4 py-2.5 pr-12 bg-gray-900/50 border border-gray-700/50 rounded-lg text-gray-200 placeholder-gray-500 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/30 font-mono text-sm"
            />
            <button
              type="button"
