...
import { Check, X, Loader2, ExternalLink, Eye, EyeOff, Search } from 'lucide-react';
import { useBrave } from '@/api/hooks';
...
export function ApiKeysSettingsPanel({ footer }: ApiKeysSettingsPanelProps) {
  const { config, loading, error: hookError, setApiKey } = useBrave();
    ...
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-medium text-gray-200">Brave Search</h3>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                {hasApiKey
                  ? 'API key configured for brave_llm_context tool'
        ...
        >
          ...
          <div className="relative">
            <input
              id="brave-api-key"
              type={masked || showApiKey ? 'text' : 'password'}
              autoComplete="off"
              ...
              placeholder={hasApiKey && !masked ? undefined : 'API key'}
          ...
          <p className="text-xs text-gray-400">
            Your API key is encrypted and stored in the local database
          </p>
        </form>
        ...
        {saveSuccess && (
          <div className="flex items-center gap-2 px-4 py-3 bg-green-500/10 border border-green-500/20 rounded-lg">
            ...
            <p className="text-sm text-green-300">API key saved successfully!</p>
          </div>
        )}
