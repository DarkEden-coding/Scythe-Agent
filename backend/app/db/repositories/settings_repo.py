...
class SettingsRepository(BaseRepository):
    ...
    def set_zai_api_key(self, api_key: str, updated_at: str) -> Settings:
        """Set the Z.ai API key (should be encrypted before calling this)."""
        settings = self.get_settings()
        if settings is None:
            raise ValueError("Settings record missing")
    ...
    def get_zai_api_key(self) -> str | None:
        """Get the Z.ai API key (encrypted)."""
        settings = self.get_settings()
        if settings is None:
            return None
