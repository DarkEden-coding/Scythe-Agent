"""Z.ai API client for chat completions and model listing.

Uses OpenAI-compatible endpoints. Supports GLM thinking control via
`thinking: { type: "enabled" | "disabled" }` for models where thinking
...
_THINKING_DEFAULT_ON_MODEL_HINTS = ("glm-4.7", "glm-5")
...
def _model_defaults_to_thinking(model: str) -> bool:
    low = model.strip().lower()
    return any(hint in low for hint in _THINKING_DEFAULT_ON_MODEL_HINTS)
    ...
    async for chunk in response.aiter_bytes(chunk_size=1024):
        ...
        while "\n" in buffer or "\r" in buffer:
            ...
            if finish_reason:
                ...
                if tool_calls_list:
                    yield StreamToolCallsEvent(type="tool_calls", tool_calls=tool_calls_list)
...
class ZAiClient:
    """Z.ai API client for model catalog and chat completion calls."""
    ...
    async def get_models(self) -> list[dict]:
        """Fetch all available models from Z.ai API."""
        ...
        for attempt in range(len(RETRY_DELAYS) + 1):
            try:
                ...
                if _should_retry(e.response.status_code) and attempt < len(RETRY_DELAYS):
                    ...
                    logger.warning(
                        "Z.ai API %s, retrying in %ss (attempt %d/%d)",
    ...
    async def create_chat_completion_stream(
        ...
        *,
        ...
        elif _model_defaults_to_thinking(model):
                    ...
                    async with client.stream(
                        ...
                        if response.status_code >= 400:
                            ...
                            try:
                                ...
                                logger.error("Z.ai API error %s: %s", response.status_code, err_json)
                            except Exception:
                                logger.error("Z.ai API error %s: %s", response.status_code, err_body[:500])
                        response.raise_for_status()
