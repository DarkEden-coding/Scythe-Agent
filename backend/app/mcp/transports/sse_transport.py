...
class MCPSSETransport:
    """Minimal SSE transport abstraction for local MVP tests."""
    ...
    async def request(self, method: str, params: dict | None = None) -> dict[str, Any]:
        if not self._connected:
            raise RuntimeError("MCP SSE transport not connected")
        ...
        if method == "tools/call":
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": json.dumps({"transport": "sse", "tool": name, "args": args}, sort_keys=True),
                    }
                ]
            }
        raise RuntimeError(f"Unsupported MCP method for SSE transport: {method}")
