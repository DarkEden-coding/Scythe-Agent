...
class MCPStdioTransport:
    """Minimal stdio transport abstraction for local MVP tests."""
        ...
        if method == "tools/call":
            ...
            return {
                "content": [
                    {
                        "type": "text",
                        "text": json.dumps({"transport": "stdio", "tool": name, "args": args}, sort_keys=True),
                    }
                ]
            }
        raise RuntimeError(f"Unsupported MCP method for stdio transport: {method}")
