"""List files tool — directory listing with depth and ignore rules."""
...
async def _handler(payload: dict, context: ToolExecutionContext) -> ToolExecutionResult:
    ...
    if not path_raw or path_raw.strip() == ".":
        ...
        path_raw = str(Path(context.project_root).resolve())
    try:
        base = resolve_path(path_raw.strip(), project_root=context.project_root)
    except ValueError as exc:
        return ToolExecutionResult(output=str(exc), file_edits=[], ok=False)
    if not base.exists() or not base.is_dir():
        return ToolExecutionResult(output=f"Directory not found: {base}", file_edits=[], ok=False)
