...
def _extract_context(content: str, start_char: int, end_char: int) -> str:
    ...
    pos = 0
    line_indices: list[tuple[int, int, int]] = []
...
async def _handler(payload: dict, context: ToolExecutionContext) -> ToolExecutionResult:
    try:
        target = resolve_path(payload.get("path", ""), project_root=context.project_root)
    ...
    if not target.exists():
        return ToolExecutionResult(output=f"File not found: {target}", file_edits=[], ok=False)

    result = await asyncio.to_thread(_edit_file_sync, target, search, replace)
